...
public class MapSwitchAction extends Activity {
	...
	*/
	public static SwitchEvent getMappedSwitchEvent(SwitchEvent switchEvent, String[] defaultMapResource){
		...
		StringBuilder switchChangesBuilder = new StringBuilder(Integer.toBinaryString(switchEvent.getSwitchChanges())).reverse();
		StringBuilder switchStatesBuilder = new StringBuilder(Integer.toBinaryString(switchEvent.getSwitchStates())).reverse();
		StringBuilder newSwitchChangesBuilder = new StringBuilder(Integer.toBinaryString(switchEvent.getSwitchChanges())).reverse();
		StringBuilder newSwitchStatesBuilder = new StringBuilder(Integer.toBinaryString(switchEvent.getSwitchStates())).reverse();
	...
	protected void handleSwitchEvent(SwitchEvent newSwitchEvent, int currCounter, int direction, boolean test) {
		...
		if(currCounter == action_array.length){
			...
			MapSwitchAction.getMappedSwitchEvent(new SwitchEvent(43, 21), getResources().getStringArray(R.array.switch_actions));
			finish();
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if(keyCode == KeyEvent.KEYCODE_DPAD_UP)
			handleSwitchEvent(null, currentCounter, 0, true);
		else if(keyCode == KeyEvent.KEYCODE_DPAD_DOWN)
			handleSwitchEvent(null, currentCounter, 1, true);
		else if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT)
			handleSwitchEvent(null, currentCounter, 2, true);
		else if(keyCode == KeyEvent.KEYCODE_DPAD_RIGHT)
			handleSwitchEvent(null, currentCounter, 3, true);
		return false;
	}
	...
	private Runnable mSwitchActionRunnable = new Runnable(){
		public void run(){
			...
			if(action_array == null){
				...
				Log.d("MapSwitchAction : map error","dismissing in error");
				finish();
				return;
			}
